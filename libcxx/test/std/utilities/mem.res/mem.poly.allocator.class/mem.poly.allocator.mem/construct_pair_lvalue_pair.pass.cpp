...
template <class Pair>
void testConstruct(const Pair& arg,
                   UsesAllocatorType expected1, UsesAllocatorType expected2)
{
    ...
    int_alloc.construct(p, arg);
    checkConstruct<const typename Pair::first_type&>(p->first, expected1);
    checkConstruct<const typename Pair::second_type&>(p->second, expected2);
...
{
    {
        ...
        ASSERT_SAME_TYPE(decltype(alloc.construct(pair_ptr, *pair_ptr)), void);
        ASSERT_NOT_NOEXCEPT(alloc.construct(pair_ptr, *pair_ptr));
    }
    {
